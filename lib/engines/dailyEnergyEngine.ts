...
const THEME_BY_PERSONAL_DAY: Record<number, { theme: string; description: string }> = {
  ...
  9: { theme: "Adaptación", description: "Un día para fluir sin resistencia y ajustarte a lo inesperado." },
...
const STRENGTHS_BY_PERSONAL_DAY: Record<number, string[]> = {
  ...
  9: ["Adaptabilidad", "Fluidez", "Lectura de situación"],
...
const CAUTIONS_BY_PERSONAL_DAY: Record<number, string[]> = {
  ...
  9: ["Identidad difusa", "Apego disfrazado", "Evitar el compromiso"],
...
function getYearTheme(year: number): string {
  const themes: Record<number, string> = {
    ...
    9: "un año de adaptación y desapego",
