...
export default function Badge({ children, variant = "accent", className = "" }: BadgeProps) {
  return (
    <span
      className={`inline-flex items-center gap-2 px-3 py-1.5 font-heading text-[0.7rem] font-semibold uppercase tracking-[0.15em] rounded-none ${variants[variant]} ${className}`}
    >
      {children}
    </span>
  );
