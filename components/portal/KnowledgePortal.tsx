...
export default function KnowledgePortal({ profile, knowledge }: KnowledgePortalProps) {
  ...
  return (
    <div className="bg-card rounded-3xl shadow-sm border border-border p-6 sm:p-8">
      <h2 className="font-serif text-2xl font-semibold text-foreground mb-6">Portal del conocimiento</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={subject.id} className="bg-background rounded-2xl p-6 hover:shadow-md transition-shadow">
            <div className="text-3xl mb-3">{subject.icon}</div>
            <h3 className="font-serif text-lg font-semibold text-foreground mb-2">{subject.title}</h3>
            <p className="text-sm text-muted mb-4">{subject.description}</p>
            <ul className="space-y-2 mb-4">
              {(knowledge[subject.id as keyof typeof knowledge] as any)?.topics?.map((topic: any) => (
                <li key={topic.title} className="text-sm text-foreground flex items-start gap-2">
                ...
                <li key={subject.id + "-no-data"} className="text-sm text-muted">Explorá este sistema simbólico en detalle.</li>
              )}
            </ul>
            <button
              onClick={() => {
