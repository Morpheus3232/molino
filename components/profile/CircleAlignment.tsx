...
export default function CircleAlignment({ profile }: CircleAlignmentProps) {
  ...
  const tensionDisplay = tension ? getZodiacDisplay(tension) : null;
        ...
        <motion.div {...reveal}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <motion.div
              ...
              className="p-6 border border-ink/10 rounded-lg bg-ink/[0.015]"
            >
            ...
            >
              ...
              {tensionDisplay ? (
                <span className="inline-flex items-center gap-2 px-4 py-2 border border-ink/10 rounded-full bg-background text-sm">
                  <span role="img" aria-label={tensionDisplay.name}>{tensionDisplay.emoji}</span>
                  <span className="font-medium">{tensionDisplay.name}</span>
                </span>
              ) : (
