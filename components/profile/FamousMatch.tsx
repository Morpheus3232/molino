...
function MatchCard({
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border p-3.5 sm:p-4 transition-all relative overflow-hidden flex flex-col justify-between ${
        matchCount >= 2
          ? "bg-gradient-to-br from-card to-background border-accent/30 shadow-md"
    ...
    >
      ...
      <div>
        ...
        {/* Quote if available */}
        {person.quote && (
          <blockquote className="mt-2.5 text-[10px] italic text-foreground/80 border-l-2 border-accent/40 pl-2 py-0.5">
      ...
      <div className="mt-3.5 pt-3 border-t border-ink/10 grid grid-cols-[2fr_1fr_1fr] gap-1 text-center">
        ...
        >
          <span className="block font-mono text-[8px] uppercase tracking-wider">Zodíaco</span>
        ...
        {/* Life Path */}
        <div className="p-1.5 rounded-md border bg-ink/[0.02] border-ink/5 text-muted">
          <span className="flex items-center justify-center gap-0.5 font-mono text-[8px] uppercase tracking-wider">
            Camino
            {matchLifePath && <Check className="w-2 h-2 text-foreground" aria-label="Coincide" />}
          </span>
          <span className={`font-mono text-[11px] font-bold ${matchLifePath ? "text-foreground" : ""}`}>
            {person.lifePath}
          </span>
        </div>
        ...
        <div className="p-1.5 rounded-md border bg-ink/[0.02] border-ink/5 text-muted">
          <span className="flex items-center justify-center gap-0.5 font-mono text-[8px] uppercase tracking-wider">
            Solar
            {matchSunSign && <Check className="w-2 h-2 text-foreground" aria-label="Coincide" />}
          </span>
          <span className={`font-mono text-[11px] font-bold truncate block ${matchSunSign ? "text-foreground" : ""}`}>
