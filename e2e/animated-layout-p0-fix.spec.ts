...
test.describe("P0 — navegación client-side no deja la página en blanco", () => {
  test("click en un link dentro de un dropdown (Explorar → Academia) renderiza contenido visible", async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/");
    const nav = page.getByRole("navigation", { name: "Navegación principal" });
    await nav.getByRole("button", { name: /explorar/i }).click();
    await page.locator("#explore-menu").getByRole("link", { name: "Academia", exact: true }).click();
